...
describe('InstanceAiSettingsService', () => {
	...
	describe('resolveModelConfig', () => {
		const user = mock<User>({ id: 'user-1', settings: {} });

		it('uses matching shared credential when no credential preference is set', async () => {
			globalConfig.instanceAi.model = 'google/gemini-2.5-flash';
			globalConfig.instanceAi.modelApiKey = '';
			credentialsFinderService.findCredentialsForUser.mockResolvedValue([
				{ id: 'cred-1', name: 'Gemini Shared', type: 'googlePalmApi' },
			] as never);
			credentialsService.decrypt.mockResolvedValue({ apiKey: 'shared-key' } as never);
		...
		it('keeps explicit credential preference when user selected one', async () => {
			globalConfig.instanceAi.model = 'google/gemini-2.5-flash';
			const preferredUser = mock<User>({
				id: 'user-2',
				settings: { instanceAi: { credentialId: 'cred-explicit', modelName: 'gemini-2.0-flash' } },
			});
			...
			credentialsService.decrypt.mockResolvedValue({ apiKey: 'explicit-key' } as never);

			const result = await service.resolveModelConfig(preferredUser);

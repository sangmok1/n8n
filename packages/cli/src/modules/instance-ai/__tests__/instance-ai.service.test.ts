...
describe('InstanceAiService — background task auto-follow-up', () => {
	...
	it('skips internal follow-up when the background task opts out of auto-follow-up', async () => {
		const { service, task, getSpawnOptions } = createBackgroundTaskFollowUpService();
		...
		await getSpawnOptions().onSettled?.({ ...task, skipAutoFollowUp: true });

		expect(service.startInternalFollowUpRun).not.toHaveBeenCalled();
		expect(service.recordBackgroundTerminalOutcome).toHaveBeenCalledWith(
			expect.objectContaining({ skipAutoFollowUp: true }),
		);
	});

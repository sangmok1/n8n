...
import {
	...
	WorkflowLoopStorage,
	ThreadTaskStorage,
	buildScaffoldWorkflowJson,
	buildWorkflowResearchContext,
	hasDispatchedWorkflowWork,
	invokeDirectWorkflowBuild,
	isWorkflowBuildRequest,
	tryBuildPatternWorkflowJson,
} from '@n8n/instance-ai';
import { setSchemaBaseDirs, type WorkflowJSON } from '@n8n/workflow-sdk';
...
function sanitizeDirectBuildMessage(message: string): string {
	return message
		.replace(/<workflow-research>[\s\S]*?<\/workflow-research>/gi, '')
		.replace(/<background-task-completed>[\s\S]*?<\/background-task-completed>/gi, '')
		.replace(/<planned-task-follow-up[\s\S]*?<\/planned-task-follow-up>/gi, '')
		.trim();
}
...
export class InstanceAiService {
	...
	private isDirectBuildSuccessful(task: ManagedBackgroundTask): boolean {
		const outcome = task.outcome as { submitted?: boolean; workflowId?: string } | undefined;
		if (outcome && typeof outcome.submitted === 'boolean') {
			return outcome.submitted === true && typeof outcome.workflowId === 'string';
		}
		return task.status === 'completed' && !task.result?.trimStart().startsWith('Error:');
	}
	...
	private buildBackgroundTerminalOutcome(task: ManagedBackgroundTask): TerminalOutcome {
		...
		const userFacingMessage = task.skipAutoFollowUp
			? status === 'completed'
				? '워크플로 생성이 완료되었습니다. 오른쪽 아티팩트 패널에서 확인해 주세요.'
				: status === 'cancelled'
					? '워크플로 생성이 취소되었습니다.'
					: '워크플로 생성에 실패했습니다. 다시 시도하거나 워크플로 목록에서 직접 만들어 주세요.'
			: status === 'completed'
	...
	private async createExecutionEnvironment(
		...
		const orchestrationContext: OrchestrationContext = {
			...
			autoApprovePlanReview: context.permissions?.createWorkflow === 'always_allow',
			modelId,
			...
			waitForConfirmation: async (requestId: string) => {
				...
				return await new Promise<ConfirmationData>((resolve) => {
					...
					queueMicrotask(() => {
						...
						// mnetplus: skip plan-review UI when workflow creation is always allowed
						if (context.permissions?.createWorkflow === 'always_allow') {
							const confirmation = this.eventBus
	...
	private async createPatternWorkflowDraft(
		threadId: string,
		message: string,
		context: InstanceAiContext,
		...
		try {
			const created = await context.workflowService.createFromWorkflowJSON(json as WorkflowJSON);
			(context.aiCreatedWorkflowIds ??= new Set<string>()).add(created.id);
			return { workflowId: created.id, name: created.name ?? json.name };
		} catch (error) {
			this.logger.warn('Failed to create pattern workflow draft', {
				threadId,
				error: error instanceof Error ? error.message : String(error),
			});
			return null;
	...
	private async createScaffoldWorkflowDraft(
		threadId: string,
		message: string,
		context: InstanceAiContext,
	): Promise<{ workflowId: string; name: string } | null> {
		try {
			...
			this.logger.warn('Failed to create scaffold workflow draft', {
				threadId,
				error: error instanceof Error ? error.message : String(error),
			});
			return null;
	...
	private publishAssistantTextDelta(
		threadId: string,
		runId: string,
		text: string,
		responseId?: string,
	...
	private async handleDirectBuildSettlement(
		user: User,
		task: ManagedBackgroundTask,
		runId: string,
	): Promise<void> {
		const outcome = task.outcome as { submitted?: boolean; workflowId?: string } | undefined;
		const adapter = this.adapterService.createContext(user, { threadId: task.threadId });

		if (outcome?.submitted === true && typeof outcome.workflowId === 'string') {
			try {
				const workflow = await adapter.workflowService.get(outcome.workflowId);
				this.publishWorkflowCreatedArtifact(task.threadId, runId, {
					id: outcome.workflowId,
					name: workflow.name ?? 'Untitled',
				});
			} catch (error) {
				this.logger.warn('Failed to publish direct-build workflow artifact', {
					threadId: task.threadId,
					workflowId: outcome.workflowId,
					error: getErrorMessage(error),
				});
			}
		} else if (task.status === 'failed' && task.directBuildMessage) {
			const pattern = await this.createPatternWorkflowDraft(
				task.threadId,
			...
			if (fallback) {
				...
				task.outcome = {
					...
					summary: `Fallback workflow created: ${fallback.name}`,
				};
				task.result = `워크플로 초안 "${fallback.name}"을(를) 만들었습니다.`;
		...
		await this.reapAiTemporaryFromRun(task.threadId, user, undefined);
	}

	private publishWorkflowCreatedArtifact(
		threadId: string,
		runId: string,
		workflow: { id: string; name: string },
	): void {
		const toolCallId = `direct-workflow-${workflow.id}`;
	...
	private async runWorkflowBuildOrScaffold(
		...
		const researchContext = await buildWorkflowResearchContext(
			context.workflowService,
			fullMessage,
		);
		const enrichedMessage = `${fullMessage}\n\n${researchContext}`;
		const directBuildMessage = sanitizeDirectBuildMessage(userMessage);

		const buildOutcome = await invokeDirectWorkflowBuild(orchestrationContext, {
			task: enrichedMessage,
			conversationContext: enrichedMessage,
			directBuildMessage,
		});
		if (buildOutcome.taskId) {
			return '워크플로 빌더가 작업 중입니다. 아래 빌드 카드와 오른쪽 아티팩트 패널에서 진행 상황을 확인해 주세요.';
		}

		this.logger.warn('Direct workflow build unavailable — creating scaffold draft', {
			threadId,
			result: buildOutcome.result,
		});

		const scaffold = await this.createScaffoldWorkflowDraft(threadId, directBuildMessage, context);
		if (scaffold) {
			return `워크플로 초안 "${scaffold.name}"을(를) 만들었습니다. 워크플로 목록에서 열어 수정해 주세요.`;
		}

		return (
			buildOutcome.result ||
			'워크플로를 만들지 못했습니다. 잠시 후 다시 시도하거나 워크플로 목록에서 직접 만들어 주세요.'
		);
	}
	...
	private async executeRun(
		...
		try {
			...
			const shouldDirectPlan =
				!isReplanFollowUp &&
				!checkpoint?.isCheckpointFollowUp &&
				!resumeReason &&
				isWorkflowBuildRequest(message);

			if (shouldDirectPlan) {
				this.publishAssistantTextDelta(
					threadId,
					runId,
					'워크플로와 템플릿을 검색한 뒤 빌드합니다…',
					`${runId}:direct-build`,
				);

				const outputText = await this.runWorkflowBuildOrScaffold(
					orchestrationContext,
					context,
					threadId,
					runId,
				...
				this.publishAssistantTextDelta(
					...
					runId,
					`\n\n${outputText}`,
					`${runId}:direct-build-result`,
				);

				this.evaluateTerminalResponse(threadId, runId, 'completed', {
					messageGroupId,
					correlationId: messageId,
				});
				await this.finalizeRunTracing(runId, tracing, {
					...
					outputText,
					modelId,
					metadata: { completion_source: 'direct-build' },
				});
				messageTraceFinalization = {
					status: 'completed',
					outputText,
					modelId,
					metadata: this.buildMessageTraceMetadata(threadId, runId, { status: 'completed' }),
				};
				...
				await this.finalizeRun(threadId, runId, 'completed', snapshotStorage, {
					userId: user.id,
					modelId,
					archivedWorkflowIds,
				});
			...
			let outputText = await (result.text ?? Promise.resolve(''));
			if (
				result.status === 'completed' &&
				isWorkflowBuildRequest(message) &&
				!hasDispatchedWorkflowWork(result.workSummary)
			) {
				this.logger.info('Orchestrator finished without workflow dispatch — running direct build', {
					threadId,
					runId,
				});
				this.publishAssistantTextDelta(
					...
					runId,
					'\n\n워크플로를 생성하는 중입니다…',
					`${runId}:build-fallback`,
				);
				outputText = await this.runWorkflowBuildOrScaffold(
					orchestrationContext,
					context,
					threadId,
					runId,
	...
	private spawnBackgroundTask(
		...
		const outcome = this.backgroundTasks.spawn({
			...
			skipAutoFollowUp: opts.skipAutoFollowUp,
			directBuildMessage: opts.directBuildMessage,
			run: opts.run,
			...
			onSettled: async (task) => {
				if (task.skipAutoFollowUp) {
					const user = this.runState.getThreadUser(opts.threadId);
					if (user) {
						await this.handleDirectBuildSettlement(user, task, runId);
					}
				}

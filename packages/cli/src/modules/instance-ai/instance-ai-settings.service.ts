...
export class InstanceAiSettingsService {
	...
	async loadFromDb(): Promise<void> {
		...
		// mnetplus: workflow building should not block on approval prompts
		this.permissions = {
			...this.permissions,
	...
	async resolveModelConfig(user: User): Promise<ModelConfig> {
		const prefs = this.readUserPreferences(user);
		const providerFromConfiguredModel = this.extractModelProvider(this.config.model);
		const explicitCredentialId = prefs.credentialId ?? null;
		const credential =
			(explicitCredentialId
				? await this.credentialsFinderService.findCredentialForUser(explicitCredentialId, user, [
						'credential:read',
					])
				: null) ??
			(await this.resolveFallbackCredentialForProvider(user, providerFromConfiguredModel));

		if (!credential) return this.envVarModelConfig();

		const provider = CREDENTIAL_TO_MODEL_PROVIDER[credential.type];
		if (!provider) return this.envVarModelConfig();
	...
	private async resolveFallbackCredentialForProvider(
		user: User,
		provider: string | null,
	): Promise<Awaited<ReturnType<CredentialsFinderService['findCredentialsForUser']>>[number] | null> {
		if (!provider) return null;
		const credentials = await this.credentialsFinderService.findCredentialsForUser(user, [
			'credential:read',
		]);
		const fallback = credentials.find((credential) => {
			const credentialProvider = CREDENTIAL_TO_MODEL_PROVIDER[credential.type];
			return credentialProvider === provider;
		});
	...
	private extractModelProvider(model: string): string | null {
		const slash = model.indexOf('/');

...
/** Find a workflow-builder sub-agent that is still running. */
export function findActiveWorkflowBuilder(
	node: InstanceAiAgentNode | undefined,
): InstanceAiAgentNode | null {
	if (!node) return null;

	for (const child of node.children) {
		if (child.role === 'workflow-builder' && child.status === 'active') {
			return child;
		}
		const nested = findActiveWorkflowBuilder(child);
...
export function hasActiveWorkflowBuilder(messages: InstanceAiMessage[]): boolean {
	for (let i = messages.length - 1; i >= 0; i--) {
		const msg = messages[i];
		if (msg.role !== 'assistant' || !msg.agentTree) continue;
		if (findActiveWorkflowBuilder(msg.agentTree)) return true;
	}
	return false;
}

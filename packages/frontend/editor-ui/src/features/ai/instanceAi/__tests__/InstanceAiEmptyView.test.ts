...
describe('InstanceAiEmptyView', () => {
	...
	it('does not auto-resume the last thread when opened with ?new=1', async () => {
		routeQuery.value = { new: '1' };
		...
		store.loadThreads.mockImplementation(async () => {
		...
		expect(replaceMock).toHaveBeenCalledWith({ name: 'InstanceAi', query: {} });

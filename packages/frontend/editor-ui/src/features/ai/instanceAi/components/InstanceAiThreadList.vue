...
function displayThreadTitle(title: string): string {
	if (!title || title === NEW_CONVERSATION_TITLE) {
		return i18n.baseText('instanceAi.thread.newConversation');
	}
	return title;
}

function startNewChat() {
	void router.push({ name: INSTANCE_AI_VIEW, query: { new: '1' } });
}
				...
				<div v-for="group in groupedThreads" :key="group.label" :class="$style.group">
					...
					>
						...
						<template v-else>
							...
							>
								<span :class="$style.threadTitle">{{ displayThreadTitle(thread.title) }}</span>
							</RouterLink>
							<N8nActionDropdown
								:items="threadActions"

...
export function getLatestBuildResult(node: InstanceAiAgentNode): BuildResult | undefined {
	...
	for (let i = node.toolCalls.length - 1; i >= 0; i--) {
		...
		if (!tc.isLoading && tc.result && typeof tc.result === 'object') {
			...
			if (tc.toolName === 'workflows' && result.workflow && typeof result.workflow === 'object') {
				const workflow = result.workflow as Record<string, unknown>;
				if (typeof workflow.id === 'string') {
					return { workflowId: workflow.id, toolCallId: tc.toolCallId };
				}
			}
		}
	}

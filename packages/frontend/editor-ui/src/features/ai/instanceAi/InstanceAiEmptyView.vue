...
import {
	INSTANCE_AI_LAST_THREAD_KEY,
	INSTANCE_AI_SKIP_RESUME_KEY,
...
onMounted(() => {
	void (async () => {
		const skipResume =
			route.query.new === '1' || sessionStorage.getItem(INSTANCE_AI_SKIP_RESUME_KEY) === '1';
		sessionStorage.removeItem(INSTANCE_AI_SKIP_RESUME_KEY);

		if (route.query.new === '1') {
			await router.replace({ name: INSTANCE_AI_VIEW, query: {} });
		}

		await store.loadThreads();

		if (!skipResume && store.threads.length > 0) {
			const lastThreadId = sessionStorage.getItem(INSTANCE_AI_LAST_THREAD_KEY);
			const target =
				(lastThreadId ? store.threads.find((thread) => thread.id === lastThreadId) : undefined) ??
				store.threads[0];

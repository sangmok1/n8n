...
import { hasActiveWorkflowBuilder } from './agentTimeline.utils';
...
export function createThreadRuntime(threadId: string, hooks: ThreadRuntimeHooks) {
	...
	const hasActiveBackgroundBuilder = computed(() => hasActiveWorkflowBuilder(messages.value));
	const isWorkflowBuildInProgress = computed(
		() => isStreaming.value || hasActiveBackgroundBuilder.value,
	);
	...
	function isAutoApprovableConfirmation(item: PendingConfirmationItem): boolean {
		const conf = item.toolCall.confirmation;
		if (conf.inputType === 'plan-review') return true;
		return isGenericApprovalEligible(item);
	}
	...
	watch(
		...
		async (items) => {
			for (const item of items) {
				...
				if (!isAutoApprovableConfirmation(item)) continue;

				const isPlanReview = conf.inputType === 'plan-review';
				if (!isPlanReview) {
					if (sessionAlwaysAllowKeys.value.size === 0) continue;
	...
	return reactive({
		...
		// computeds
		isStreaming,
		hasActiveBackgroundBuilder,
		isWorkflowBuildInProgress,
		isSendingMessage,
		hasMessages,
		isHydratingThread,
		producedArtifacts,

...
const createTab = (
	...
	const localizedLabel =
		locale.locale === 'ko' && routeKey === 'workflows' ? '워크플로' : locale.baseText(label);
...
const options = computed<Array<TabOptions<string>>>(() => {
	...
	if (props.additionalTabs?.length) {
		...
		for (const processed of processedAdditionalTabs) {
			const localizedProcessed =
				locale.locale === 'ko' && processed.label === 'Data tables'
					? { ...processed, label: '데이터 테이블' }
					: processed;

			const { insertAfter, ...tab } = processed;
			const { ...localizedTab } = localizedProcessed;
			const anchorIndex = insertAfter ? tabs.findIndex((t) => t.value === insertAfter) : -1;
			if (anchorIndex !== -1) {
				tabs.splice(anchorIndex + 1, 0, localizedTab);
			} else {
				tabs.push(localizedTab);
			}
		}
	}

	...
	return [
		...
		},
		{
			...
			guard: (user) =>
				...
				user.id !== usersStore.currentUserId &&
				(user as { disabled?: boolean }).disabled === true,
		},
		{
...
async function onApproveSignup(userId: string) {
	try {
		await usersStore.approveSignupRequest({ id: userId });
		await updateUsersTableData(usersTableState.value);
		showToast({
			type: 'success',
			title: '가입 승인 완료',
			message: '이제 해당 사용자가 로그인할 수 있습니다.',
		});
	} catch (error) {
		showError(error, '가입 승인 처리에 실패했습니다');
	}
}

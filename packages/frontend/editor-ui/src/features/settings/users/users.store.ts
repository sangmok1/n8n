...
export const useUsersStore = defineStore(STORES.USERS, () => {
	...
	const requestSignup = async (params: { loginId: string; password: string }) => {
		return await makeRestApiRequest<{ submitted: boolean }>(
	...
	const showPersonalizationSurvey = async () => {
		// mnetplus: disable first-login personalization survey modal
		return;
	};
	...
	const approveSignupRequest = async ({ id }: { id: string }) => {
		const user = await makeRestApiRequest<IUserResponse>(
			rootStore.restApiContext,
			'PATCH',

...
import { useUiLocale, type UiLocale } from '@/app/composables/useUiLocale';
import { useGeminiInstanceAi, DEFAULT_GEMINI_MODEL } from '@/app/composables/useGeminiInstanceAi';
...
const {
	canConfigure: canConfigureGemini,
	isSaving: isSavingGemini,
	isConfigured: isGeminiConfigured,
	configuredModelName: geminiModelName,
	geminiApiKey,
	loadStatus: loadGeminiStatus,
	saveApiKey: saveGeminiApiKey,
} = useGeminiInstanceAi();
const currentSelectedTheme = ref(useUIStore().theme);
const languageOptions: Array<{ value: UiLocale; label: BaseTextKey }> = [
	{ value: 'en', label: 'settings.personal.language.en' },
	{ value: 'ko', label: 'settings.personal.language.ko' },
];
...
const canSaveChanges = computed(() => {
	if (!hasAnyChanges.value) return false;
	// Theme/language-only changes should not be blocked by basic-info form validity.
	if (!hasAnyBasicInfoChanges.value) return true;
	return readyToSubmit.value;
});
...
function onSaveClick() {
	if (!hasAnyBasicInfoChanges.value && hasAnyPersonalisationChanges.value) {
		void updatePersonalisationSettings()
			.then(() => {
				showToast({
					title: i18n.baseText('settings.personal.personalSettingsUpdated'),
					message: '',
			...
			.catch((error) => {
				showError(error, i18n.baseText('settings.personal.personalSettingsUpdatedError'));
			});
		return;
	}
...
async function onSaveGeminiClick() {
	await saveGeminiApiKey(geminiApiKey.value);
}
...
<template>
	<div :class="$style.container" data-test-id="personal-settings-container">
		...
		<div>
			...
			<div v-if="canConfigureGemini" :class="$style.personalisationField">
				<N8nHeading size="small" :class="$style.geminiHeading">
					{{ i18n.baseText('settings.personal.gemini.title') }}
				</N8nHeading>
				<N8nText :bold="false" :class="$style.infoText">
					{{ i18n.baseText('settings.personal.gemini.description') }}
				</N8nText>
				<N8nInputLabel
					:class="$style.geminiField"
					:label="i18n.baseText('settings.personal.gemini.apiKey')"
				>
					<N8nInput
						...
						:placeholder="
							isGeminiConfigured
								? i18n.baseText('settings.personal.gemini.apiKey.configured')
								: i18n.baseText('settings.personal.gemini.apiKey.placeholder')
						"
						data-test-id="gemini-api-key-input"
					/>
				</N8nInputLabel>
				<N8nInputLabel
					:class="$style.geminiField"
					:label="i18n.baseText('settings.personal.gemini.model')"
				>
					<N8nInput
						v-model="geminiModelName"
						size="small"
						:placeholder="DEFAULT_GEMINI_MODEL"
						data-test-id="gemini-model-input"
					/>
				</N8nInputLabel>
				<N8nButton
					:class="$style.geminiSaveButton"
					variant="subtle"
					:label="i18n.baseText('settings.personal.gemini.save')"
					:loading="isSavingGemini"
					:disabled="!geminiApiKey.trim()"
					data-test-id="gemini-api-key-save"
					@click="onSaveGeminiClick"
				/>
			</div>

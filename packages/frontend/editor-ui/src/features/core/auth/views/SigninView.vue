...
const emailLabel = computed(() => {
	if (isLdapLoginEnabled.value && ldapLoginLabel.value) {
		return ldapLoginLabel.value;
	}
	return locale.baseText('auth.loginId');
});
...
<template>
	<div>
		...
		/>
		<div :class="$style.signupRequestLink">
			<router-link :to="{ name: VIEWS.SIGNUP_REQUEST }"> 가입 신청하기 </router-link>
		</div>
		<MfaView
			v-if="showMfaView"

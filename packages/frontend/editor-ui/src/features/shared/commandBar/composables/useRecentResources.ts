...
export function useRecentResources() {
	...
	function removeStaleWorkflow(workflowId: string): void {
		recentWorkflows.value = recentWorkflows.value.filter((w) => w.id !== workflowId);

		if (recentNodes.value[workflowId]) {
			const { [workflowId]: _removed, ...remainingNodes } = recentNodes.value;
			recentNodes.value = remainingNodes;
		}
	}
			...
			workflowsToFetch.map(async (recentWorkflow) => {
				try {
					...
					if ((error as { httpStatusCode?: number }).httpStatusCode === 404) {
						removeStaleWorkflow(recentWorkflow.id);
					}
				}
			}),
		);

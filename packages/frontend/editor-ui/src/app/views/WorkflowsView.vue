...
const selectedHomeProject = computed(() => {
	const homeProjectId = filters.value.homeProject;
	if (!homeProjectId) return null;

	return projectsStore.availableProjects.find((project) => project.id === homeProjectId) ?? null;
});

const folderTargetProject = computed(() => {
	const routeProjectId = route.params.projectId as string | undefined;
	const personalProject = projectsStore.personalProject;

	if (projectPages.isOverviewSubPage) {
		return selectedHomeProject.value ?? personalProject;
	}

	if (routeProjectId) {
		return (
			projectsStore.availableProjects.find((project) => project.id === routeProjectId) ??
			currentProject.value
		);
	}

	return selectedHomeProject.value ?? currentProject.value ?? personalProject;
});

const hasPermissionToCreateFolders = computed(() => {
	const project = folderTargetProject.value;
...
const currentBreadcrumbsProject = computed(() => folderTargetProject.value);
...
const showAddFolderButton = computed(
	() =>
		!readOnlyEnv.value &&
		hasPermissionToCreateFolders.value &&
		(showRegisteredCommunityCTA.value || showFolders.value),
);
...
<template>
	...
	>
		...
		<template v-if="showAddFolderButton" #add-button>

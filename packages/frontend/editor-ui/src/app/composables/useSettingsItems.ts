...
/** mnetplus: settings sidebar entries hidden from the UI. */
const MNETPLUS_HIDDEN_SETTINGS_IDS = new Set([
...
function isMnetplusHiddenSetting(id: string): boolean {
	return MNETPLUS_HIDDEN_SETTINGS_IDS.has(id);
}
...
export function useSettingsItems() {
	...
	const settingsItems = computed<IMenuItem[]>(() => {
		const menuItems: IMenuItem[] = [
			{
				...
				position: 'top',
				available:
					!isMnetplusHiddenSetting('settings-usage-and-plan') &&
					canUserAccessRouteByName(VIEWS.USAGE),
			...
			},
			{
				...
				position: 'top',
				available:
					!isMnetplusHiddenSetting('settings-external-secrets') &&
					canUserAccessRouteByName(VIEWS.EXTERNAL_SECRETS_SETTINGS),
			...
			},
			{
				...
				position: 'top',
				available:
					!isMnetplusHiddenSetting('settings-source-control') &&
					canUserAccessRouteByName(VIEWS.SOURCE_CONTROL),
			...
			},
			{
				...
				position: 'top',
				available:
					!isMnetplusHiddenSetting('settings-ldap') &&
					canUserAccessRouteByName(VIEWS.LDAP_SETTINGS),
		...
		menuItems.push({
			...
			position: 'top',
			available:
				!isMnetplusHiddenSetting('settings-log-streaming') &&
				canUserAccessRouteByName(VIEWS.LOG_STREAMING_SETTINGS),

...
export interface SpawnBackgroundTaskOptions {
	...
	/**
	 * When true, the background task settlement path must not auto-resume the
	 * orchestrator with `<background-task-completed>`. Used for direct
	 * chat-to-workflow builds where creation alone is the deliverable.
	 */
...
export interface OrchestrationContext {
	...
	timeZone?: string;
	/** Skip plan-review HITL and auto-approve planner output (host sets from permissions). */
	autoApprovePlanReview?: boolean;
}

...
export interface StartBuildWorkflowAgentInput {
	...
	workflowId?: string;
	conversationContext?: string;
	directBuildMessage?: string;
	workItemId?: string;
	taskId?: string;
	agentId?: string;
	plannedTaskId?: string;
	/** Skip orchestrator auto-follow-up after the builder settles (build-only mode). */
	skipAutoFollowUp?: boolean;
}
...
export async function startBuildWorkflowAgentTask(
	...
	if (useSandbox) {
		...
		if (context.workflowTaskService && context.domainContext && !input.skipAutoFollowUp) {
	...
	const spawnOutcome = spawnBackgroundTask({
		...
		skipAutoFollowUp: input.skipAutoFollowUp,
		directBuildMessage: input.skipAutoFollowUp
			? sanitizeDirectBuildMessage(input.directBuildMessage ?? input.task)
			: undefined,
		run: async (
			signal,
			drainCorrections,
			...
			await withTraceContextActor(traceContext, async () => {
				...
				if (!fallbackMainWorkflowId) {
					const text = toolFinalText.trimStart().startsWith('Error:')
						? toolFinalText
						: 'Error: workflow builder finished without creating a workflow.';
					return {
						text,
						outcome: buildNotSubmittedOutcome(workItemId, context.runId, taskId, text),
					};
				}
				return {
					text: toolFinalText,
					outcome: withDeterministicRouting({
						...
						runId: context.runId,
						taskId,
						workflowId: fallbackMainWorkflowId,
						submitted: true,
						triggerType: 'manual_or_testable',
						needsUserInput: false,
						summary: toolFinalText,
					}),
				};
			}),
	});

...
- Never reference \`$json\` directly in JavaScript code passed to \`expr(...)\`. This is invalid: \`expr('prefix ' + JSON.stringify($json))\`. Use a single expression string instead: \`expr('{{ \"prefix \" + JSON.stringify($json) }}')\`.
...
Default behavior is build-first: when credentials are missing or ambiguous, continue building and submit the workflow structure without asking the user credential questions. Ask credential/setup questions only when the user explicitly requests credential configuration now.

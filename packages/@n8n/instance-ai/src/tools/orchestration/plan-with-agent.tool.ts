...
import {
	createAddPlanItemTool,
	createRemovePlanItemTool,
	publishPlanUpdate,
} from './add-plan-item.tool';
	...
	return new Tool('plan')
		...
		.handler(async (input: { guidance?: string }) => {
			...
			try {
				...
				if (
					context.autoApprovePlanReview &&
					!accumulator.isEmpty() &&
					context.plannedTaskService &&
					context.schedulePlannedTasks
				) {
					...
					return {
						result: `Plan auto-approved and ${taskCount} task${taskCount === 1 ? '' : 's'} dispatched.`,
					};
				}

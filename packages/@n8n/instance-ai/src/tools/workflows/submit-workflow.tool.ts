...
function enhanceBuildErrors(errors: string[]): string[] {
	...
	if (!needsTemplateGuidance && !needsJsonExprGuidance) return errors;

	const guidance = [...errors];
	if (needsJsonExprGuidance) {
		guidance.push(
			'Expression guidance: do not reference $json directly inside JavaScript passed to expr(...). Use a single expression string, e.g. expr(\'{{ "Results:\\n" + JSON.stringify($json, null, 2) }}\').',
		);
	}
...
function stripAllNodeCredentials(json: WorkflowJSON): number {
	let strippedNodes = 0;
	for (const node of json.nodes ?? []) {
		if (!node.credentials) continue;
		const credentials = node.credentials as Record<string, unknown>;
			...
			async ({ filePath: rawFilePath, workflowId, projectId, name }: SubmitWorkflowInput) => {
				...
				if (formattedValidationErrors.length > 0) {
					informational.push({
						...
						message:
							'Schema validation reported issues, but saved workflow structure anyway so you can finish setup in the editor.',
					});
				}
				...
				if (!workflowId) {
					json.name = buildAiWorkflowDefaultName();
				} else if (name) {
					json.name = name;
				}
				...
				// Save
				let savedId: string;
				let savedWithCredentialsStripped = false;
				try {
					if (workflowId) {
					...
					const saveErrorMessage = error instanceof Error ? error.message : 'Unknown error';
					const saveErrorText = saveErrorMessage.toLowerCase();
					if (isCredentialSaveFailure(saveErrorText)) {
						...
						if (strippedNodeCount > 0) {
							...
							try {
								...
								savedWithCredentialsStripped = true;
							} catch (retryError) {
								const errors = [
							...
							const errors = [`Workflow save failed: ${saveErrorMessage}`];
							const remediation = classifySubmitFailure(errors, 'workflow_save_failed');
							await reportAttempt({ success: false, errors, remediation });
							return {
								success: false,
				...
				if (savedWithCredentialsStripped) {
					informational.push({
						code: 'CREDENTIALS_STRIPPED',
						message:
							'Saved workflow structure without credentials. Open the workflow and add credentials before running or publishing.',
					});
				}

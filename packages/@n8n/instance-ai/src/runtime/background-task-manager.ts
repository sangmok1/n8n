...
export interface ManagedBackgroundTask {
	...
	/** When true, settlement must not trigger orchestrator auto-follow-up. */
	skipAutoFollowUp?: boolean;
	/** Original user build prompt for direct-build settlement fallbacks. */
	directBuildMessage?: string;
}

function isFailedBackgroundTaskResult(raw: string | BackgroundTaskResult): boolean {
	const text = typeof raw === 'string' ? raw : raw.text;
	if (text.trimStart().startsWith('Error:')) {
		return true;
	}

	const outcome = typeof raw === 'string' ? undefined : raw.outcome;
	return outcome?.submitted === false;
}

export function resolveBackgroundTaskCompletion(
	raw: string | BackgroundTaskResult,
): Pick<ManagedBackgroundTask, 'status' | 'result' | 'outcome' | 'error'> {
	const result = typeof raw === 'string' ? raw : raw.text;
	const outcome = typeof raw === 'string' ? undefined : raw.outcome;

	if (isFailedBackgroundTaskResult(raw)) {
		return {
			status: 'failed',
			result,
			outcome,
...
export class BackgroundTaskManager {
	...
	spawn(options: SpawnManagedBackgroundTaskOptions): SpawnManagedBackgroundTaskResult {
		...
		const task: ManagedBackgroundTask = {
			...
			skipAutoFollowUp: options.skipAutoFollowUp,
			directBuildMessage: options.directBuildMessage,
		};
	...
	private async executeTask(
		...
		try {
			...
			const completion = resolveBackgroundTaskCompletion(raw);

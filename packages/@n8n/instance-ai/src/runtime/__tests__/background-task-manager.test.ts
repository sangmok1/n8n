...
import {
	...
	resolveBackgroundTaskCompletion,
...
describe('BackgroundTaskManager', () => {
	...
	describe('spawn', () => {
		...
		it('calls onCompleted with structured result', async () => {
			...
			resolve({ text: 'summary', outcome: { workflowId: 'wf-1', submitted: true } });
			await flushPromises();
			...
			expect(onCompleted).toHaveBeenCalledWith(
				expect.objectContaining({
					status: 'completed',
					result: 'summary',
					outcome: { workflowId: 'wf-1', submitted: true },
				}),
			);
		});

		it('calls onFailed when structured result reports submitted=false', async () => {
			const onCompleted = jest.fn();
			...
			resolve({
				text: 'Error: workflow builder finished without submitting /src/workflow.ts.',
				outcome: { submitted: false, summary: 'not submitted' },
			});
			await flushPromises();
...
describe('resolveBackgroundTaskCompletion', () => {
	it('marks Error-prefixed text as failed', () => {
		expect(resolveBackgroundTaskCompletion('Error: no workflow')).toEqual(
			expect.objectContaining({ status: 'failed' }),
		);
	});

	it('marks submitted=false outcomes as failed', () => {
		expect(
			resolveBackgroundTaskCompletion({
				text: 'Builder stopped early',

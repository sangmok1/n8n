...
describe('handleBuildOutcome', () => {
	...
	it('allows two pre-save submit failures before blocking the third', () => {
		...
		expect(third.state.preSaveSubmitFailures).toBe(3);
	...
	it('counts failed post-submit repair submits and blocks when the second budget is exhausted', () => {
		const state = makeState({
			runId: 'run_1',
			phase: 'repairing',
			workflowId: 'wf_123',
...
describe('retry policy', () => {
	it('blocks on repeated needs_patch with same failureSignature after five repairs', () => {
		const state = makeState({ phase: 'verifying', workflowId: 'wf_123', rebuildAttempts: 1 });
		const priorAttempts: AttemptRecord[] = Array.from({ length: 5 }, (_, idx) => ({
			workItemId: 'wi_test',
			phase: 'repairing',
			attempt: idx + 1,
			action: 'patch',
	...
	it('blocks on repeated rebuild with same failureSignature after five repairs', () => {
		const state = makeState({ phase: 'verifying', workflowId: 'wf_123', rebuildAttempts: 1 });
		const priorAttempts: AttemptRecord[] = Array.from({ length: 5 }, (_, idx) => ({
			workItemId: 'wi_test',
			phase: 'repairing',
	...
	it('blocks when patch follows repeated rebuilds with the same failureSignature', () => {
		const state = makeState({ phase: 'verifying', workflowId: 'wf_123', rebuildAttempts: 1 });
		const priorAttempts: AttemptRecord[] = Array.from({ length: 5 }, (_, idx) => ({
			workItemId: 'wi_test',
			phase: 'repairing',
	...
	it('blocks duplicate failure signatures without consuming submit budget', () => {
		const state = makeState({
			phase: 'verifying',
			workflowId: 'wf_123',
			successfulSubmitSeen: true,

import { containsHangul, truncateToTitle } from '../title-utils';

describe('containsHangul', () => {
	it('detects Korean text', () => {
		expect(containsHangul('디스코드 워크플로')).toBe(true);
	});

	it('returns false for English-only text', () => {
		expect(containsHangul('Build a Slack workflow')).toBe(false);
	});
});
